...
def process(arr, n,m):
    #Converting pos into neg, then neg to pos
    ans = 0
    Q = []
    curr = 0
    ...
    if m>1 and arr[m-1]>0:
        ans += 1
    
    ans1 = (m-1-neg) + (n-m-pos)        #Converting pos into neg, then neg to pos
    ans2 = neg + (n-m-pos)              #Coverting neg into pos, then neg to pos
