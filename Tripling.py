...
def process(A, B, C, n):
    ...
    for i in range(n-1, -1, -1):
        for j in range(n-1, -1, -1):
            for k in range(n-1, -1, -1):
                ...
                dp[i][j][k] = dp[i][j][k+1]+dp[i][j+1][k]+dp[i+1][j][k]+dp[i+1][j+1][k+1]
    ...
    @cache
    def dfs(i,j,k):
        if i==n or j==n or k==n: return 0
        
        ans = dfs(i+1, j, k) + dfs(i, j+1, k) + dfs(i, j, k+1) + dfs(i+1, j+1, k+1)
        ans -= dfs(i+1, j+1, k) + dfs(i+1, j, k+1) + dfs(i, j+1, k+1)
        if A[i]==B[j]==C[k]:
            ans += 1 + dfs(i+1, j+1, k+1)
