...
N = 10**6+1
PT = PrimeTable(N)
newprimes = PT.primes[:430]
newtable = [1]*(N)
for i in range(2, N):
    if newtable[i]>1: continue
...
def division(ele):
    ...
    largest = newtable[ele]
    while largest!=ele:
        factors.add(largest)
        ele//=largest
        largest = newtable[ele]
    
    if ele>1: factors.add(ele)
    return factors
...
def process(arr, n):    
    ...
    for k in range(n):
        i, j = randint(0,len(arr)-1), randint(0, len(arr)-1)
    ...
    for ele in arr:
        for p in division(ele):
            
            if p==1: continue
            if p in whole: return yes
            whole.add(p)
