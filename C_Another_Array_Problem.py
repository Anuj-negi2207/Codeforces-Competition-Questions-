...
def process(arr, n):
    if n<3: return max(sum(arr), 2*abs(arr[0]-arr[1]))
    a = max(arr[2:])
    b = max(arr[:-2])
    c = max(abs(arr[0]-arr[1]), abs(arr[-2]-arr[-1]))
    return max(sum(arr), a*n, b*n, c*n)
    stack = []
