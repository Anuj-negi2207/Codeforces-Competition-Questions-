...
factinv[-1] = exponentiation(fact[-1], mod-2, mod)
...
def process(arr):
    n, k = arr
    if k==0: return exponentiation(3, n)
    elif k==n: return exponentiation(3, n-1)
    elif k==1: return fastMul(exponentiation(3, n-2), (n+2))
    elif k==(n-1): return fastMul(exponentiation(3, n-3), (n+10))
    ...
    for i in range(1, k+1):
        if k+i <= n:
            a,b = C(n-k, i) * C(k-1, i-1) % mod, power[n-(k+i)] * power[k-i] % mod
            #print(a,b)
            ans += fastMul(a,b)
        if k+i-1 <= n:
            a, b = C(n-k, i-1) * C(k-1, i-1) % mod  , power[n-(k+i-1)] * power[k-i] % mod
            #print(a,b)
            ans += a * b % mod
        ans %= mod
    return ans
